...
pt<-st_read("./gis/random_points2/RanPts_RSonly_19x22_NoOlango2.shp")%>%
...
pt2 <- pt%>%
  st_join(hab)%>%
  dplyr::select(-Shape_Leng,-Shape_Area,-PtID,-Id_Hab1,-Id,-Id_geom)%>%
  glimpse()
...
st_write(pt3,"./results_train/hab_Resil_Pts_RS.gpkg", delete_layer=T)
