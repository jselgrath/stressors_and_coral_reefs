...
pts<-st_read("./results_test/IndpVarOther_Pts_RS.gpkg")%>%
  glimpse()
plot(pts)
...
f1<-read_csv("./results_test/2_pts_FishingImpact_normalized.csv")%>% 
  glimpse()

# Cumulative fishing years, or with lag
# select for values standardized against max for all years (A)
f2<- read_csv("./results_test/3_pts_FishingYrs_2cumulative.csv")%>% glimpse()

# g1n measures
# cumulative
f3<- read_csv("./results_test/4_pts_cumulative_fishing_g1n_blast.csv")%>% 
  glimpse()

f4<- read_csv("./results_test/4_pts_cumulative_fishing_g1n_kaykay.csv")%>%
  glimpse()

f5<- read_csv("./results_test/4_pts_cumulative_fishing_g1n_poison.csv")%>%
  glimpse()

f6<- read_csv("./results_test/5_pts_FishingYrsDest_normalized.csv")%>%
  glimpse()


f7<- read_csv("./results_test/6_pts_FishingYrs_destructive_cumulative.csv")%>% 
  glimpse()


f8<-read_csv("./results_test/7_pts_fragstats.csv")%>%
  glimpse()

f9<- read_csv("./results_test/11_pts_PopRsk_Norm.csv")%>% 
  glimpse()
...
pts2<-f1%>% #f7p, f4p,f5p,,f17p,f18p,f19p,f20p,f21p,f22p, f11p, f12p,f13p,f6p,
  ...
  full_join(f9)%>%
  full_join(f10)%>%
  # full_join(f11)%>% # not using distance categories for habitats
  dplyr::select(-geom)%>%
  glimpse()
...
# geopackage
st_write(pts3,"./results_test/13_IndpVar_Pts_all.gpkg", delete_layer=T)

# csv
write_csv(pt4,"./results_test/13_IndpVar_Pts_all.csv")
