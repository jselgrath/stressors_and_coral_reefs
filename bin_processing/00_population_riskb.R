...
# points of villages with 2010 population, updated area, interview statistics
# updated barangay area data from 2023 - from https://data.humdata.org/dataset/cod-ab-phl
village1<-st_read("./gis2/population/village_pop_fishing_area_2025.shp")%>% # villages up to 10 km from focal area
  dplyr::select(barangay=BgyName,fishers_n,interviews,interviews_n=intervie_1,area_km2_2025=AREA_SQKM,pop_2010,ID_0:validOn)%>%
  dplyr::filter(barangay!="Fatima", barangay!="Campao Oriental")%>%
  mutate(pop_dens_km2_2025 = round(pop_2010 / area_km2_2025,2))%>%
  glimpse()  # not coastal, few fishers

range(village1$pop_dens_km2_2025)
...
# full spatial area - usng official area from 2023 philippines file
