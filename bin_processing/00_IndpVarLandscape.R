...
analyze_patches <- function(sf_obj, hab_col, target_crs) {
  ...
  merged2 <- merged %>%
    mutate(
      patch_area      = st_area(geom),
      patch_perimeter = st_length(st_cast(geom, "MULTILINESTRING")),
      shape_index     = as.numeric((0.25 * patch_perimeter) / sqrt(patch_area)) # patch shape index metric is from fragstats
    )
...
d3a<- d3%>%
  mutate(patch_shape_index=round(as.numeric(shape_index),2),
         ...
         patch_dist_to_same_hab_m=round(as.numeric(nearest_same_hab_dist),2))%>% # euclidian nearest neighbor
  dplyr::select(-patch_area,-patch_perimeter,-nearest_same_hab_dist,-shape_index) %>%
  filter(hab_reclass!="Cloud")%>%
  glimpse()
...
length(unique(d3a$patch_id)) # all unique

# save as geotiff & .shp  ---------------------------------
st_write(d3a,"./results/landscape.gpkg",layer="habitat_all_db_landscape1", delete_layer = TRUE)

st_write(d3a,"./gis2/landscape/habitat_all_db_landscape1.shp", delete_layer = TRUE)

# create a readmefile for shapefile -----------------
readme_text <- c("readme",
                 "Jennifer Selgrath",
                 "",
                 "The file /gis2/landscape/habitat_all_db_landscape1.shp is based on ./results/habitat.gpkg,layer=habitat_all_db_reclass2. Calculating patch metrics for reclassified habitats based on 2025 confusion matrix. See code in 00_IndpVarLandscape.R",
                 "Geopackage version has full variable names in results/landscape.gpkg")

# Write to a text file
writeLines(readme_text, con = "./gis2/landscape/readme_landscape1.txt")
