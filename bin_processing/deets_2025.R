theme_deets <- function(base_size = 14,
                        ...
                        legend_pos = c(0.8, 0.25),   # if legend = "inside"
                        legend_bg = "transparent",
                        legend_border = NA,
                        strip_blank = FALSE,
                        rotate_x = 0,
                        rotate_y = 0,
                        center_x = FALSE,           # NEW toggle
                        center_y = FALSE) {         # NEW toggle
  ...
  th <- theme_bw(base_size = base_size) +
    theme(
      ...
      # Rotate + align axis labels
      axis.text.x = element_text(
        angle = rotate_x,
        hjust = ifelse(center_x, 0.5, 1),
        vjust = 0.5
      ),
      axis.text.y = element_text(
        angle = rotate_y,
        hjust = ifelse(center_y, 0.5, 1),
        vjust = 0.5
      )
    )
...
# $ rotate 90
# theme_deets(rotate_x = 90)

# rotate and center
# p1 + theme_deets(rotate_x = 90, center_x = TRUE)