...
d1<-st_read("./results/landscape.gpkg","habitat_all_db_landscape1")%>%
  glimpse()
# plot(d1)
d1
...
calc_target_distances <- function(sf_obj, hab_col = "hab_reclass",
  ...
  tgt3    <- sf_proj %>% filter(.data[[hab_col]] == target3)
  ...
  sources$patch_dist_to_coral_m    <- round(set_units(nearest_dist(sources, tgt3), "m"),2)
  return(sources)
}
...
# run function ------------------------------------
# d3 is  merged habitat sf with hab_reclass
...
d2 <- calc_target_distances(d1,
                            ...
                            target3     = "Coral")    # target habitat 3
...
st_write(d3,"./results/landscape.gpkg",layer="habitat_all_db_landscape2", delete_layer = TRUE)

# create a readmefile for shapefile -----------------
readme_text <- c("readme",
                 "Jennifer Selgrath",
                 "",
                 "The file /gis2/landscape/habitat_all_db_landscape2.shp is based on ./results/habitat.gpkg,layer=habitat_all_db_reclass2 and ./results/landscape.gpkg,habitat_all_db_landscape1. See code in 00_IndpVarLandscape2.R. The goal is to calc distance between coral/rubble patches and sg/mg/coral",
                 "Geopackage version has full variable names in results/landscape.gpkg")

# Write to a text file
writeLines(readme_text, con = "./gis2/landscape/readme_landscape2.txt")
