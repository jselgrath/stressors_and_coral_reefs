...
# est$ordr<--1*(c(1,4,7,5,8,2,6,3)) # pos to neg
est$ordr<--1*(c(1,3,5,4,6,2,8,7)) # pos to neg, interactions at bottom
est%>%dplyr::select(ordr, Parameter)%>%arrange(ordr)
...
ggsave("./doc/fig_3b_reduced_model.tif", width = 8, height=4)
