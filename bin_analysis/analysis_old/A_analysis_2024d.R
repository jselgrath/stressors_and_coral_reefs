...
X <- model.matrix(Reef_state ~ 
                    Depth_m +
                    I(sg_minDist_100^2)+ # MAKE SURE THAT THIS IS NOT EXPONENTATED TWICE BELOW, DO I NEED TO LOG IT BELOW TO GET THE COEFFICENT?
                    SHAPE+
                    PopRskDecay.Nrm+
                    fYrLag30A+
                    cum_FA_blast10+
...
d5<-d4%>%
  mutate(Depth=cs.(Depth_m)[,1], # [,1] calls it out of the matrix
       ...
       Fishing_legacy_1980_2000=cs.(fYrLag30A)[,1],
       Blast_fishing_2010_2000=cs.(cum_FA_blast10)[,1],
       Geomorphic=Geomorphic2)%>%
  select(
    Reef_state,Depth, Seagrass_isolation,Patch_shape,Population_risk, 
    Fishing_legacy_1980_2000,Blast_fishing_2010_2000,Geomorphic,MPA,Ecological_zone=ecological_zone)%>%
  glimpse()
...
# -------------------------
# save final models --------------------------
save(m.me_52,file="./results_train/mixedEf1_all.R") 
save(m.me_50,file="./results_train/mixedEf1_no_landscape.R") 


#save image to set options for probabilities in GT
save.image("./results_train/mixedEf1.RData")
